...
//                 {split.currency}{expense.amount.toFixed(2)}
//               </Text>
//             </TouchableOpacity>
//           ))
//         )}
...
import { db } from '@/firebaseConfig';
import { useAuth } from '@/components/AuthContext';
import AddExpenseModal from './AddExpenseModal'; // Import the AddExpenseModal
...
export default function SplitDetailsScreen({ splitId, onBack }: SplitDetailsScreenProps) {
  ...
  const [totalExpenses, setTotalExpenses] = useState(0);
  const [userExpenses, setUserExpenses] = useState(0);
  const [showAddExpenseModal, setShowAddExpenseModal] = useState(false); // New state for modal
  const { user } = useAuth();
